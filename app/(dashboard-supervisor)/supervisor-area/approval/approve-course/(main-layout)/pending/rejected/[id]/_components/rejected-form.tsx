...
interface CourseRejectFormProps {
  id: string
  uuid: string
}

export function CourseRejectForm({ id, uuid }: CourseRejectFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        form.reset()
        router.push("/supervisor-area/approval/approve-course/pending?page=1")
        router.refresh()
      } else {
        toast({
          title: "Gagal",
          description: "Pengajuan gagal ditolak",
          variant: "destructive",
        })
      }
    } catch (error) {
